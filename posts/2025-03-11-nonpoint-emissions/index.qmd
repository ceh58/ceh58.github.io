...
description: "A breakdown of the *drivers* behind hidden emissions."
author:
  - name: Carmen Hoyt
...
I chose three fundamental graphic forms (a bar chart, a donut chart, and a map), created them in `{ggplot}`, and assembled them using Affinity. To answer “*who* is responsible for nonpoint emissions,” I used a donut chart to breakdown total nonpoint emissions (in tons) by sector. This lead me to find that the majority of emissions come from the **Mobile** sector, which I further broke down into two categories (**Onroad** and **Nonroad**). To investigate the relative contribution of each category, I used a bar chart to visually compare lengths (representative of tons of emissions). Originally, I had planned to use three separate bars as “smokestacks”, but I changed my mind because I felt that smokestacks better represented *point* sources better than nonpoint sources. I also used donut charts to answer the question “*what* pollutants comprise nonpoint emissions.” After finding that CO2 was the vast majority by weight (98%), I made a second donut chart to break down the remaining pollutants. Finally, I determined that the best way to answer “*where* are nonpoint emissions concentrated” was to show the spatial allocation of emissions (in tons) per square mile on a stylized map of the U.S. with states shaded by magnitude. I chose to normalize by area (square mile) to give a better sense of how states related to their emissions contributions given their size. 


I used text annotations to give context and  point out the main takeaways for each of my three questions/sections. I labeled the bars on the bar chart to distinguish between the two **Mobile** sector categories and annotated with the total tons of emissions for each. I also included simple text legends for my donut charts, with the text colored to match the chart. I chose to add a subtitle to the map to make it clear what the units (tons/square mile) were. 
...
[^2]: To give this better context, the brown box behind "nonpoint" represents 2% of the total page space, proportional to the contribution of nonpoint emissions to total emissions.
...
road_data <- emissions_cleaned_non_point %>%
  ...
  # Label by "Mobile" sectory subcategories
...
pollutants_2 <- ggplot(other_pollutants, aes(x = 2, y = emissions_tons, fill = category)) +
  ...
  scale_fill_manual(values = paletteer::paletteer_d("palettetown::gloom")[-c(1,2,3,4)])
